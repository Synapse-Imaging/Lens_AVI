...
/////////////////////////////////////////////////////////////////////////////
// CInspectAdminViewHideDlg2 dialog

CInspectAdminViewHideDlg2* CInspectAdminViewHideDlg2::m_pInstance = NULL;

CInspectAdminViewHideDlg2* CInspectAdminViewHideDlg2::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance) {
		m_pInstance = new CInspectAdminViewHideDlg2();
		if (!m_pInstance->m_hWnd) {
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_INSPECT_ADMIN_VIEW_HIDE_DLG2, pFrame->GetActiveView());
...
void CInspectAdminViewHideDlg2::DeleteInstance()
{
...
void CInspectAdminViewHideDlg2::Show()
{
...
CInspectAdminViewHideDlg2::CInspectAdminViewHideDlg2(CWnd* pParent /*=NULL*/)
	: CDialog(CInspectAdminViewHideDlg2::IDD, pParent)
{
	//{{AFX_DATA_INIT(CInspectAdminViewHideDlg2)
...
void CInspectAdminViewHideDlg2::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CInspectAdminViewHideDlg2)
	DDX_Control(pDX, IDC_VSCRBAR, m_VScrBar);
	DDX_Control(pDX, IDC_HSCRBAR, m_HScrBar);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CInspectAdminViewHideDlg2, CDialog)
	//{{AFX_MSG_MAP(CInspectAdminViewHideDlg2)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
...
/////////////////////////////////////////////////////////////////////////////
// CInspectAdminViewHideDlg2 message handlers

void CInspectAdminViewHideDlg2::ClearAll()
{
	m_dZoomRatio = 0.4;
	m_iImageWidth = THEAPP.m_ModelDefineInfo.m_iVisionImageWidth[VISION_NUMBER_2];
	m_iImageHeight = THEAPP.m_ModelDefineInfo.m_iVisionImageHeight[VISION_NUMBER_2];
...
BOOL CInspectAdminViewHideDlg2::PreTranslateMessage(MSG* pMsg)
{
	switch (pMsg->message) {
...
BOOL CInspectAdminViewHideDlg2::OnInitDialog()
{
...
BOOL CInspectAdminViewHideDlg2::OnEraseBkgnd(CDC* pDC)
{
	pDC->FillSolidRect(m_ClientRect, TS_COLOR_EXTRA_BG);
	return TRUE;
}

void CInspectAdminViewHideDlg2::OnPaint()
{
...
void CInspectAdminViewHideDlg2::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void CInspectAdminViewHideDlg2::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (HideWindowHandle < 0 || !m_bDisplayImage) return;
...
void CInspectAdminViewHideDlg2::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void CInspectAdminViewHideDlg2::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (!m_bDisplayImage) return;
...
void CInspectAdminViewHideDlg2::OnMouseMove(UINT nFlags, CPoint point)
{
	if (!m_bDisplayImage) return;
...
void CInspectAdminViewHideDlg2::OnDestroy()
{
	CDialog::OnDestroy();
...
void CInspectAdminViewHideDlg2::OnSize(UINT nType, int cx, int cy)
{
...
BOOL CInspectAdminViewHideDlg2::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
...
BOOL CInspectAdminViewHideDlg2::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	switch (m_ToolBarState)
	{
	case TS_MOVE:
...
void CInspectAdminViewHideDlg2::Reset()
{
	InitViewRect();
	UpdateViewportManager();
}

void CInspectAdminViewHideDlg2::InitViewRect()
{
	m_ViewRect.left = 0;
	m_ViewRect.top = 0;
...
void CInspectAdminViewHideDlg2::UpdateViewportManager()
{
...
void CInspectAdminViewHideDlg2::OverlaySetViewportManager(int iLeft, int iTop, double dZoomRatio)
{
...
void CInspectAdminViewHideDlg2::OverlaySetCropViewportManager(CPoint cpCenterPoint, int iWidth, int iHeight)
{
...
void CInspectAdminViewHideDlg2::OverlaySetCenterViewportManager(int iCenterX, int iCenterY, double dZoomRatio)
{
...
void CInspectAdminViewHideDlg2::ViewRectSet()
{
...
void CInspectAdminViewHideDlg2::ScrollBarSet()
{
	SCROLLINFO Info;
...
void CInspectAdminViewHideDlg2::ScrollBarPosSet()
{
...
void CInspectAdminViewHideDlg2::DrawActiveTRegion()
{
...
void CInspectAdminViewHideDlg2::DrawSelectPartTRegion()
{
	CDC* pDC = GetDC();
...
void CInspectAdminViewHideDlg2::ZoomInSelectedPart(double dZoomRatio, int iStartPosX, int iStartPosY)
{
	if (m_bDisplayImage) {
		m_dZoomRatio = dZoomRatio;
...
void CInspectAdminViewHideDlg2::OnTbDraw()
{
	m_ToolBarState = TS_DRAW;
}

void CInspectAdminViewHideDlg2::OnTbMove()
{
	m_ToolBarState = TS_MOVE;
}

void CInspectAdminViewHideDlg2::OnTbSelectPart()
{
	m_ToolBarState = TS_SELECT_PART;
}

void CInspectAdminViewHideDlg2::OnTbZoomIn()
{
	if (m_bDisplayImage) {
...
void CInspectAdminViewHideDlg2::OnTbZoomOut()
{
	if (m_bDisplayImage) {
		m_dZoomRatio = (int)(m_dZoomRatio * 10) * 0.1;
...
void CInspectAdminViewHideDlg2::OnTbFitWidth()
{
	if (m_bDisplayImage) {
...
void CInspectAdminViewHideDlg2::OnTbFitHeight()
{
	if (m_bDisplayImage) {
...
int CInspectAdminViewHideDlg2::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	{
		strLog.Format("Halcon Exception [InspectAdminViewHideDlg2::OnCreate] : %s", (CString)(LPCTSTR)except.ErrorMessage());
...
void CInspectAdminViewHideDlg2::LoadScanImage(int nScanNum)
{

}

void CInspectAdminViewHideDlg2::MoveToDefectArea()
{

}

void CInspectAdminViewHideDlg2::RedrawView()
{
	InvalidateRect(NULL, FALSE);
}

void CInspectAdminViewHideDlg2::RedrawRect()
{
	InvalidateRect(&m_ClientRect, FALSE);
}

void CInspectAdminViewHideDlg2::DrawFrameActiveTRegion()
{
	CDC* pDC = GetDC();
...
void CInspectAdminViewHideDlg2::OnTbLive()
{
}

void CInspectAdminViewHideDlg2::set_display_font(HalconCpp::HTuple WindowHandle, HalconCpp::HTuple Size, HalconCpp::HTuple Font,
...
int CInspectAdminViewHideDlg2::WriteResultFile(int iInspectionType, int iPcVisionNo, int iVisionCamType, int iResultImageNo, HObject HDisplayImage, HObject HInspectionTypeAlgoDetectionDefectRgn, int* piDefectCodeIndex, CString sBarcodeName)
{
	...
	try
	{
		...
		*piDefectCodeIndex = -1;
		...
		{
			strLog.Format("CInspectAdminViewHideDlg2::WriteResultFile ==> Invalid Image");
			...
			(THEAPP.Struct_PreferenceStruct.m_bUseResultImageThreadOnlyGood && !bResultGood && bBarcodeResultNG))
		{
			if (THEAPP.Struct_PreferenceStruct.m_bResultImageSaveLocal)
			{
				...
				if (sDefectResultFile.GetLength() == 0)
				{
					if (THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage)
						WriteImage(HImgDump_save, "jpg", 0, HTuple(THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName));
				}
				else
					WriteImage(HImgDump_save, "jpg", 0, HTuple(THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile));
				...
				if (sDefectResultFile.GetLength() == 0)
				{
					if (THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage)
					{
						strSaveFileName = THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName;
				...
				{
					strSaveFileName = THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile;
					strLog.Format("Save result image, Time(ms): %d, File name: %s", log_time_ms, strSaveFileName);
					THEAPP.m_log_image->info("{}", LOG_CSTR(strLog));
				}
			}
			...
			if (THEAPP.Struct_PreferenceStruct.m_bResultImageSaveLAS)
			{
				...
				{
					if (THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage)
						WriteImage(HImgDump_save, "jpg", 0, HTuple(THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName));
				}
				else
					WriteImage(HImgDump_save, "jpg", 0, HTuple(THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile));
				auto log_time_end = std::chrono::high_resolution_clock::now();
				auto log_time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(log_time_end - log_time_start).count();
				...
				if (sDefectResultFile.GetLength() == 0)
				{
					if (THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage)
					{
						strSaveFileName = THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName;
						strLog.Format("Save result image, Time(ms): %d, File name: %s", log_time_ms, strSaveFileName);
						THEAPP.m_log_image->info("{}", LOG_CSTR(strLog));
					}
				}
				...
				{
					strSaveFileName = THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile;
					strLog.Format("Save result image, Time(ms): %d, File name: %s", log_time_ms, strSaveFileName);
					THEAPP.m_log_image->info("{}", LOG_CSTR(strLog));
				}
			}
		...
		else
		{
			if (THEAPP.Struct_PreferenceStruct.m_bResultImageSaveLocal)
			{
				if ((THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage && sDefectResultFile.GetLength() == 0) || sDefectResultFile.GetLength() != 0)
				{
					...
					if (sDefectResultFile.GetLength() == 0)
						pSaveThreadIDParam->sSavePath = THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName;
					else
						pSaveThreadIDParam->sSavePath = THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile;
					THEAPP.m_pInspectService->AddListSaveResultImageParam(pSaveThreadIDParam, VISION_NUMBER_2, m_pMzNo, m_pTrayNo, m_pModuleNo);
				}
			}
			...
			if (THEAPP.Struct_PreferenceStruct.m_bResultImageSaveLAS)
			{
				if ((THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage && sDefectResultFile.GetLength() == 0) || sDefectResultFile.GetLength() != 0)
				{
					...
					if (sDefectResultFile.GetLength() == 0)
						pSaveThreadIDParam_Dual->sSavePath = THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName;
					else
						pSaveThreadIDParam_Dual->sSavePath = THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_2][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile;
					THEAPP.m_pInspectService->AddListSaveResultImageParam(pSaveThreadIDParam_Dual, VISION_NUMBER_2, m_pMzNo, m_pTrayNo, m_pModuleNo);
				}
			}
		}
	...
	catch (HException& except)
	{
		...
		strLog.Format("Halcon Exception [InspectAdminViewHideDlg2::WriteResultFile] : <%s>%s", sOperatorName, sErrMsg);
		THEAPP.m_log_halcon->warn("{}", LOG_CSTR(strLog));
...
void CInspectAdminViewHideDlg2::DecideInspectResult(int iInspectionType)
{
	if (m_iInspectResult == DEFECT_TYPE_GOOD)
		return;
...
int CInspectAdminViewHideDlg2::WriteSelectedRosReviewFile(int iPcVisionNo, int iVisionCamType, int iNoInspectionImage, HObject* pHDisplayImage, HObject* pHDefectRgn_ImageNo, CString sBarcodeID, int* piNoSaveReviewImage)
{
	(void)iVisionCamType;
	...
	*piNoSaveReviewImage = 0;
	...
	try
	{
		...
		{
			strLog.Format("CInspectAdminViewHideDlg2::WriteSelectedRosReviewFile ==> Invalid Image");
			THEAPP.m_log_inspection->warn("{}", LOG_CSTR(strLog));
			return 0;
		}
		...
		for (size_t iGroup = 0; iGroup < vecReviewGroup.size(); iGroup++)
		{
			...
			else
			{
				if (THEAPP.Struct_PreferenceStruct.m_bReviewImageSaveLocal)
				{
					...
					THEAPP.m_pInspectService->AddListSaveReviewImageParam(pSaveThreadIDParam, VISION_NUMBER_2, m_pMzNo, m_pTrayNo, m_pModuleNo);
				}
				...
				if (THEAPP.Struct_PreferenceStruct.m_bReviewImageSaveLAS)
				{
					...
					THEAPP.m_pInspectService->AddListSaveReviewImageParam(pSaveThreadIDParam_Dual, VISION_NUMBER_2, m_pMzNo, m_pTrayNo, m_pModuleNo);
				}
			}
	...
	catch (HException& except)
	{
		...
		strLog.Format("Halcon Exception [InspectAdminViewHideDlg2::WriteSelectedRosReviewFile] : <%s>%s", sOperatorName, sErrMsg);
		THEAPP.m_log_halcon->warn("{}", LOG_CSTR(strLog));
...
int CInspectAdminViewHideDlg2::WriteSelectedRosReviewFile_FAI(int iMzNo, int iTrayNo, int iModuleNo, int iPcVisionNo, int iVisionCamType, int iNoInspectionImage, HObject* pHDisplayImage, HObject(*ppHReviewRgn_FAI_Item)[10], POINT(*ppiViewportCenter_FAI_Item)[10], CString sBarcodeID, int* piNoSaveReviewImage)
{
	CString sVisionCamType_Short;
	sVisionCamType_Short = THEAPP.m_ModelDefineInfo.m_strVisionName_Short[iPcVisionNo];
	...
	*piNoSaveReviewImage = 0;
	...
	try
	{
		if (THEAPP.m_pGFunction->ValidHImage(pHDisplayImage[0]) == FALSE)
		{
			strLog.Format("CInspectAdminViewHideDlg2::WriteSelectedRosReviewFile_FAI ==> Invalid Image");
			THEAPP.m_log_inspection->warn("{}", LOG_CSTR(strLog));
			return 0;
		}
		...
		for (int iFaiItemIdx = 0; iFaiItemIdx < MAX_FAI_ITEM; iFaiItemIdx++)
		{
			// AlgorithmThreadDefine 에서 검사했는데 또 해?
			BOOL bFaiItemInspected = THEAPP.m_pDualModelDataManager[iDualModelData][iPcVisionNo]->m_bMInspectFai[iFaiItemIdx];
			if (bFaiItemInspected == FALSE)
				continue;

			if (THEAPP.Struct_PreferenceStruct.m_bUseSaveFaiReviewImage == FALSE)
			{
				if (CFAIDataManager::GetInstance().GetMeasure(iMzNo, iTrayNo, iModuleNo, FAI_PARAMETER_NUMBER_1).m_bParamResultNG[iFaiItemIdx] == FALSE)
					continue;
			}

			if (THEAPP.m_Struct_FAI_ReViewImageInfo[iFaiItemIdx].m_bGenReviewImage == FALSE)
				continue;

			// WTF?
			iNoCheckFaiParameter = 1;

			// temporary refectoring - 260108, jhkim
			// 리뷰이미지 남기는 코드
			for (int iFaiParamIndex = 0; iFaiParamIndex < iNoCheckFaiParameter; iFaiParamIndex++)
			{
				bAngleMeasure = useFaiAngleMeasure && (iFaiItemIdx == 11);

				GenEmptyObj(&HConcatImage);

				UINT iTotalReviewImg = THEAPP.m_Struct_FAI_ReViewImageInfo[iFaiItemIdx].m_iNoReviewImage;
				for (int iReviewImgIdx = 0; iReviewImgIdx < iTotalReviewImg; iReviewImgIdx++)
				{
					// iFaiParamIndex 는 0으로 고정임 (???)
					UINT iReviewFAIIdx = (iFaiParamIndex * MAX_FAI_REVIEW_IMAGE) + iReviewImgIdx;
					HObject HTempValidHXLD = ppHReviewRgn_FAI_Item[iFaiItemIdx][iReviewFAIIdx];

					BOOL bValidHXLD = THEAPP.m_pGFunction->ValidHXLD(HTempValidHXLD);
					if (bValidHXLD == FALSE)
						continue;

					// m_iViewportCenter : How to assign the center of viewport (0: Blob Center, 1: Calculated Point)
					int iViewportCenter = THEAPP.m_Struct_FAI_ReViewImageInfo[iFaiItemIdx].m_iViewportCenter[iReviewImgIdx];
					if (iViewportCenter == 1) // Calculated Point
					{
						iViewportCenterX = ppiViewportCenter_FAI_Item[iFaiItemIdx][iReviewFAIIdx].x;
						iViewportCenterY = ppiViewportCenter_FAI_Item[iFaiItemIdx][iReviewFAIIdx].y;
					...
					{
						GenRegionContourXld(ppHReviewRgn_FAI_Item[iFaiItemIdx][iReviewFAIIdx], &HContourRgn, "filled");
					...
					OverlaySetCenterViewportManager(iViewportCenterX, iViewportCenterY, THEAPP.m_Struct_FAI_ReViewImageInfo[iFaiItemIdx].m_fZoomRatio[iReviewImgIdx]);

					ClearWindow(HideWindowHandle);

					HalconCpp::SetColor(HideWindowHandle, "red");

					iDisplayImageIndex = THEAPP.m_Struct_FAI_ReViewImageInfo[iFaiItemIdx].m_iImageNo[iReviewImgIdx] - 1;
					...
					HalconCpp::DispObj(ppHReviewRgn_FAI_Item[iFaiItemIdx][iReviewFAIIdx], HideWindowHandle);

					//////////////////////////////////////////////////////////////////////////

					// HALCON 으로 Dump Winodw 만들고 저장하는 듯 하다.
				...
				}	// for (int iReviewImgIdx = 0; iReviewImgIdx < THEAPP.m_Struct_FAI_ReViewImageInfo[iFaiItemIdx].m_iNoReviewImage; iReviewImgIdx++)
				...
				if (THEAPP.m_pGFunction->ValidHImage(HConcatImage))
				{
					...
					strSpec.Format("%s", THEAPP.m_Struct_FAI_ReViewImageInfo[iFaiItemIdx].m_sFaiName);

					if (CFAIDataManager::GetInstance().GetMeasure(iMzNo, iTrayNo, iModuleNo, iFaiParamIndex).m_bParamResultNG[iFaiItemIdx] == TRUE)
					{
					...
					if (bAngleMeasure)
						strSpec.Format("%s_A(%.3lf)", strSpec, CFAIDataManager::GetInstance().GetMeasure(iMzNo, iTrayNo, iModuleNo, iFaiParamIndex).m_dFAIMeasureValue[iFaiItemIdx][0]);
					else
						strSpec.Format("%s_L(%.3lf)", strSpec, CFAIDataManager::GetInstance().GetMeasure(iMzNo, iTrayNo, iModuleNo, iFaiParamIndex).m_dFAIMeasureValue[iFaiItemIdx][0]);
	...
	catch (HException& except)
	{
		...
		strLog.Format("Halcon Exception [InspectAdminViewHideDlg2::WriteSelectedRosReviewFile_FAI] : <%s>%s", sOperatorName, sErrMsg);
		THEAPP.m_log_halcon->warn("{}", LOG_CSTR(strLog));
...
BOOL CInspectAdminViewHideDlg2::ImageCheck(CString sImagePath)
{
	try
	{
		HObject HCheckImage;
