...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewOverayImageDlg3 dialog

CInspectViewOverayImageDlg3* CInspectViewOverayImageDlg3::m_pInstance = NULL;

CInspectViewOverayImageDlg3* CInspectViewOverayImageDlg3::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance) {
		m_pInstance = new CInspectViewOverayImageDlg3();
		if (!m_pInstance->m_hWnd) {
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_INSPECT_VIEW_OVERAY_IMAGE3_DLG, pFrame->GetActiveView());
...
void CInspectViewOverayImageDlg3::DeleteInstance()
{
...
void CInspectViewOverayImageDlg3::Show()
{
	ShowWindow(SW_HIDE);
...
CInspectViewOverayImageDlg3::CInspectViewOverayImageDlg3(CWnd* pParent /*=NULL*/)
	: CDialog(CInspectViewOverayImageDlg3::IDD, pParent)
{
	//{{AFX_DATA_INIT(CInspectViewOverayImageDlg3)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
	SetPosition(VIEW1_DLG1_LEFT + VIEW1_DLG1_WIDTH + 285, VIEW1_DLG1_TOP + (VIEW1_DLG1_HEIGHT + 370) / 2 + 6 + VIEW_TOOLBAR_HEIGHT, (VIEW1_DLG1_WIDTH + 280) / 2 - 18, (VIEW1_DLG1_HEIGHT + 370) / 2 - 3 - VIEW_TOOLBAR_HEIGHT);
	...
	m_sWindowName = _T("Overlay Image 2nd");
	m_iCurResultNo = 2;

	ClearAll();
}

void CInspectViewOverayImageDlg3::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CInspectViewOverayImageDlg3)
	DDX_Control(pDX, IDC_VSCRBAR, m_VScrBar);
	DDX_Control(pDX, IDC_HSCRBAR, m_HScrBar);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CInspectViewOverayImageDlg3, CDialog)
	//{{AFX_MSG_MAP(CInspectViewOverayImageDlg3)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewOverayImageDlg3 message handlers

void CInspectViewOverayImageDlg3::ClearAll()
{
	m_dZoomRatio = 0.4;
...
BOOL CInspectViewOverayImageDlg3::PreTranslateMessage(MSG* pMsg)
{
	switch (pMsg->message) {
...
BOOL CInspectViewOverayImageDlg3::OnInitDialog()
{
...
BOOL CInspectViewOverayImageDlg3::OnEraseBkgnd(CDC *pDC)
{
	pDC->FillSolidRect(m_ClientRect, TS_COLOR_EXTRA_BG);
	return TRUE;
}

void CInspectViewOverayImageDlg3::OnPaint()
{
...
void CInspectViewOverayImageDlg3::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void CInspectViewOverayImageDlg3::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (m_lWindowID < 0 || !m_bDisplayImage) return;
...
void CInspectViewOverayImageDlg3::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void CInspectViewOverayImageDlg3::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (!m_bDisplayImage) return;
...
void CInspectViewOverayImageDlg3::OnMouseMove(UINT nFlags, CPoint point)
{
	if (!m_bDisplayImage) return;
...
void CInspectViewOverayImageDlg3::OnDestroy()
{
	CDialog::OnDestroy();
...
void CInspectViewOverayImageDlg3::OnSize(UINT nType, int cx, int cy)
{
...
BOOL CInspectViewOverayImageDlg3::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	ScreenToClient(&pt);
...
BOOL CInspectViewOverayImageDlg3::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	switch (m_ToolBarState) {
	case TS_MOVE:
		if (m_bOnMoving)
...
void CInspectViewOverayImageDlg3::Reset()
{
	InitViewRect();
	UpdateViewportManager();
}

void CInspectViewOverayImageDlg3::InitViewRect()
{
	m_ViewRect.left = 0;
	m_ViewRect.top = 0;
...
void CInspectViewOverayImageDlg3::UpdateViewportManager()
{
...
void CInspectViewOverayImageDlg3::ViewRectSet()
{
...
void CInspectViewOverayImageDlg3::ScrollBarSet()
{
	SCROLLINFO Info;
...
void CInspectViewOverayImageDlg3::ScrollBarPosSet()
{
...
void CInspectViewOverayImageDlg3::DrawActiveTRegion()
{
...
void CInspectViewOverayImageDlg3::DrawSelectPartTRegion()
{
	CDC *pDC = GetDC();
...
void CInspectViewOverayImageDlg3::ZoomInSelectedPart(double dZoomRatio, int iStartPosX, int iStartPosY)
{
	if (m_bDisplayImage) {
...
void CInspectViewOverayImageDlg3::TbDraw()
{
	m_ToolBarState = TS_DRAW;
}

void CInspectViewOverayImageDlg3::TbMove()
{
	m_ToolBarState = TS_MOVE;
}

void CInspectViewOverayImageDlg3::TbSelectPart()
{
	m_ToolBarState = TS_SELECT_PART;
}

void CInspectViewOverayImageDlg3::TbZoomIn()
{
	if (m_bDisplayImage) {
...
void CInspectViewOverayImageDlg3::TbZoomOut()
{
	if (m_bDisplayImage) {
		m_dZoomRatio = (int)(m_dZoomRatio * 10) * 0.1;
...
void CInspectViewOverayImageDlg3::TbFitWidth()
{
	if (m_bDisplayImage)
	{
...
void CInspectViewOverayImageDlg3::TbFitHeight()
{
	if (m_bDisplayImage)
	{
		ClearWindow(m_lWindowID);
...
BOOL CInspectViewOverayImageDlg3::ShowWindow(int nCmdShow)
{
	return CDialog::ShowWindow(nCmdShow);
}

int CInspectViewOverayImageDlg3::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CInspectViewOverayImageDlg3::LoadScanImage(int nScanNum)
{

}

void CInspectViewOverayImageDlg3::MoveToDefectArea()
{

}

void CInspectViewOverayImageDlg3::RedrawView()
{
	InvalidateRect(NULL, FALSE);
}

void CInspectViewOverayImageDlg3::RedrawRect()
{
	InvalidateRect(&m_ClientRect, FALSE);
}

void CInspectViewOverayImageDlg3::UpdateView(BOOL bDelete)
{
	InvalidateRect(&m_ClientRect, bDelete);
}

void CInspectViewOverayImageDlg3::CallFitImageHeight()
{
	TbFitHeight();
}

void CInspectViewOverayImageDlg3::CallFitImageWidth()
{
	TbFitWidth();
}

void CInspectViewOverayImageDlg3::OnContextMenu(CWnd* pWnd, CPoint point)
{
	CMenu *pSubMenu;
...
void CInspectViewOverayImageDlg3::UpdateResultImage()
{
