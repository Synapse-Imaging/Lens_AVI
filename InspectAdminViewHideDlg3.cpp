...
/////////////////////////////////////////////////////////////////////////////
// CInspectAdminViewHideDlg3 dialog

CInspectAdminViewHideDlg3* CInspectAdminViewHideDlg3::m_pInstance = NULL;

CInspectAdminViewHideDlg3* CInspectAdminViewHideDlg3::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance) {
		m_pInstance = new CInspectAdminViewHideDlg3();
		if (!m_pInstance->m_hWnd) {
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_INSPECT_ADMIN_VIEW_HIDE_DLG3, pFrame->GetActiveView());
...
void CInspectAdminViewHideDlg3::DeleteInstance()
{
...
void CInspectAdminViewHideDlg3::Show()
{
...
CInspectAdminViewHideDlg3::CInspectAdminViewHideDlg3(CWnd* pParent /*=NULL*/)
	: CDialog(CInspectAdminViewHideDlg3::IDD, pParent)
{
	//{{AFX_DATA_INIT(CInspectAdminViewHideDlg3)
...
void CInspectAdminViewHideDlg3::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CInspectAdminViewHideDlg3)
	DDX_Control(pDX, IDC_VSCRBAR, m_VScrBar);
	DDX_Control(pDX, IDC_HSCRBAR, m_HScrBar);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CInspectAdminViewHideDlg3, CDialog)
	//{{AFX_MSG_MAP(CInspectAdminViewHideDlg3)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
...
/////////////////////////////////////////////////////////////////////////////
// CInspectAdminViewHideDlg3 message handlers

void CInspectAdminViewHideDlg3::ClearAll()
{
	m_dZoomRatio = 0.4;
	m_iImageWidth = THEAPP.m_ModelDefineInfo.m_iVisionImageWidth[VISION_NUMBER_3];
	m_iImageHeight = THEAPP.m_ModelDefineInfo.m_iVisionImageHeight[VISION_NUMBER_3];
...
BOOL CInspectAdminViewHideDlg3::PreTranslateMessage(MSG* pMsg)
{
	switch (pMsg->message) {
...
BOOL CInspectAdminViewHideDlg3::OnInitDialog()
{
...
BOOL CInspectAdminViewHideDlg3::OnEraseBkgnd(CDC* pDC)
{
	pDC->FillSolidRect(m_ClientRect, TS_COLOR_EXTRA_BG);
	return TRUE;
}

void CInspectAdminViewHideDlg3::OnPaint()
{
...
void CInspectAdminViewHideDlg3::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void CInspectAdminViewHideDlg3::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (HideWindowHandle < 0 || !m_bDisplayImage) return;
...
void CInspectAdminViewHideDlg3::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void CInspectAdminViewHideDlg3::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (!m_bDisplayImage) return;
...
void CInspectAdminViewHideDlg3::OnMouseMove(UINT nFlags, CPoint point)
{
	if (!m_bDisplayImage) return;
...
void CInspectAdminViewHideDlg3::OnDestroy()
{
	CDialog::OnDestroy();
...
void CInspectAdminViewHideDlg3::OnSize(UINT nType, int cx, int cy)
{
...
BOOL CInspectAdminViewHideDlg3::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
...
BOOL CInspectAdminViewHideDlg3::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	switch (m_ToolBarState)
	{
	case TS_MOVE:
...
void CInspectAdminViewHideDlg3::Reset()
{
	InitViewRect();
	UpdateViewportManager();
}

void CInspectAdminViewHideDlg3::InitViewRect()
{
	m_ViewRect.left = 0;
	m_ViewRect.top = 0;
...
void CInspectAdminViewHideDlg3::UpdateViewportManager()
{
...
void CInspectAdminViewHideDlg3::OverlaySetViewportManager(int iLeft, int iTop, double dZoomRatio)
{
...
void CInspectAdminViewHideDlg3::OverlaySetCropViewportManager(CPoint cpCenterPoint, int iWidth, int iHeight)
{
...
void CInspectAdminViewHideDlg3::OverlaySetCenterViewportManager(int iCenterX, int iCenterY, double dZoomRatio)
{
...
void CInspectAdminViewHideDlg3::ViewRectSet()
{
...
void CInspectAdminViewHideDlg3::ScrollBarSet()
{
	SCROLLINFO Info;
...
void CInspectAdminViewHideDlg3::ScrollBarPosSet()
{
...
void CInspectAdminViewHideDlg3::DrawActiveTRegion()
{
...
void CInspectAdminViewHideDlg3::DrawSelectPartTRegion()
{
	CDC* pDC = GetDC();
...
void CInspectAdminViewHideDlg3::ZoomInSelectedPart(double dZoomRatio, int iStartPosX, int iStartPosY)
{
	if (m_bDisplayImage) {
		m_dZoomRatio = dZoomRatio;
...
void CInspectAdminViewHideDlg3::OnTbDraw()
{
	m_ToolBarState = TS_DRAW;
}

void CInspectAdminViewHideDlg3::OnTbMove()
{
	m_ToolBarState = TS_MOVE;
}

void CInspectAdminViewHideDlg3::OnTbSelectPart()
{
	m_ToolBarState = TS_SELECT_PART;
}

void CInspectAdminViewHideDlg3::OnTbZoomIn()
{
	if (m_bDisplayImage) {
...
void CInspectAdminViewHideDlg3::OnTbZoomOut()
{
	if (m_bDisplayImage) {
		m_dZoomRatio = (int)(m_dZoomRatio * 10) * 0.1;
...
void CInspectAdminViewHideDlg3::OnTbFitWidth()
{
	if (m_bDisplayImage) {
...
void CInspectAdminViewHideDlg3::OnTbFitHeight()
{
	if (m_bDisplayImage) {
...
int CInspectAdminViewHideDlg3::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	{
		strLog.Format("Halcon Exception [InspectAdminViewHideDlg3::OnCreate] : %s", (CString)(LPCTSTR)except.ErrorMessage());
...
void CInspectAdminViewHideDlg3::LoadScanImage(int nScanNum)
{

}

void CInspectAdminViewHideDlg3::MoveToDefectArea()
{

}

void CInspectAdminViewHideDlg3::RedrawView()
{
	InvalidateRect(NULL, FALSE);
}

void CInspectAdminViewHideDlg3::RedrawRect()
{
	InvalidateRect(&m_ClientRect, FALSE);
}

void CInspectAdminViewHideDlg3::DrawFrameActiveTRegion()
{
	CDC* pDC = GetDC();
...
void CInspectAdminViewHideDlg3::OnTbLive()
{
}

void CInspectAdminViewHideDlg3::set_display_font(HalconCpp::HTuple WindowHandle, HalconCpp::HTuple Size, HalconCpp::HTuple Font,
...
int CInspectAdminViewHideDlg3::WriteResultFile(int iInspectionType, int iPcVisionNo, int iVisionCamType, int iResultImageNo, HObject HDisplayImage, HObject HInspectionTypeAlgoDetectionDefectRgn, int* piDefectCodeIndex, CString sBarcodeName)
{
	...
	try
	{
		...
		*piDefectCodeIndex = -1;
		...
		{
			strLog.Format("CInspectAdminViewHideDlg3::WriteResultFile ==> Invalid Image");
			...
			(THEAPP.Struct_PreferenceStruct.m_bUseResultImageThreadOnlyGood && !bResultGood && bBarcodeResultNG))
		{
			if (THEAPP.Struct_PreferenceStruct.m_bResultImageSaveLocal)
			{
				...
				if (sDefectResultFile.GetLength() == 0)
				{
					if (THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage)
						WriteImage(HImgDump_save, "jpg", 0, HTuple(THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName));
				}
				else
					WriteImage(HImgDump_save, "jpg", 0, HTuple(THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile));
				...
				if (sDefectResultFile.GetLength() == 0)
				{
					if (THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage)
					{
						strSaveFileName = THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName;
				...
				{
					strSaveFileName = THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile;
					strLog.Format("Save result image, Time(ms): %d, File name: %s", log_time_ms, strSaveFileName);
					THEAPP.m_log_image->info("{}", LOG_CSTR(strLog));
				}
			}
			...
			if (THEAPP.Struct_PreferenceStruct.m_bResultImageSaveLAS)
			{
				...
				{
					if (THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage)
						WriteImage(HImgDump_save, "jpg", 0, HTuple(THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName));
				}
				else
					WriteImage(HImgDump_save, "jpg", 0, HTuple(THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile));
				auto log_time_end = std::chrono::high_resolution_clock::now();
				auto log_time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(log_time_end - log_time_start).count();
				...
				if (sDefectResultFile.GetLength() == 0)
				{
					if (THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage)
					{
						strSaveFileName = THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName;
						strLog.Format("Save result image, Time(ms): %d, File name: %s", log_time_ms, strSaveFileName);
						THEAPP.m_log_image->info("{}", LOG_CSTR(strLog));
					}
				}
				...
				{
					strSaveFileName = THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile;
					strLog.Format("Save result image, Time(ms): %d, File name: %s", log_time_ms, strSaveFileName);
					THEAPP.m_log_image->info("{}", LOG_CSTR(strLog));
				}
			}
		...
		else
		{
			if (THEAPP.Struct_PreferenceStruct.m_bResultImageSaveLocal)
			{
				if ((THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage && sDefectResultFile.GetLength() == 0) || sDefectResultFile.GetLength() != 0)
				{
					...
					if (sDefectResultFile.GetLength() == 0)
						pSaveThreadIDParam->sSavePath = THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName;
					else
						pSaveThreadIDParam->sSavePath = THEAPP.m_FileBase.m_strResultImageFolder_Local[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile;
					THEAPP.m_pInspectService->AddListSaveResultImageParam(pSaveThreadIDParam, VISION_NUMBER_3, m_pMzNo, m_pTrayNo, m_pModuleNo);
				}
			}
			...
			if (THEAPP.Struct_PreferenceStruct.m_bResultImageSaveLAS)
			{
				if ((THEAPP.Struct_PreferenceStruct.m_bSaveResultGoodImage && sDefectResultFile.GetLength() == 0) || sDefectResultFile.GetLength() != 0)
				{
					...
					if (sDefectResultFile.GetLength() == 0)
						pSaveThreadIDParam_Dual->sSavePath = THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName;
					else
						pSaveThreadIDParam_Dual->sSavePath = THEAPP.m_FileBase.m_strResultImageFolder_LAS[VISION_NUMBER_3][m_pMzNo - 1][m_pTrayNo - 1] + "\\" + Filename + "_" + sBarcodeName + "_" + sDefectResultFile;
					THEAPP.m_pInspectService->AddListSaveResultImageParam(pSaveThreadIDParam_Dual, VISION_NUMBER_3, m_pMzNo, m_pTrayNo, m_pModuleNo);
				}
			}
		}
	...
	catch (HException& except)
	{
		...
		strLog.Format("Halcon Exception [InspectAdminViewHideDlg3::WriteResultFile] : <%s>%s", sOperatorName, sErrMsg);
		THEAPP.m_log_halcon->warn("{}", LOG_CSTR(strLog));
...
void CInspectAdminViewHideDlg3::DecideInspectResult(int iInspectionType)
{
	if (m_iInspectResult == DEFECT_TYPE_GOOD)
		return;
...
int CInspectAdminViewHideDlg3::WriteSelectedRosReviewFile(int iPcVisionNo, int iVisionCamType, int iNoInspectionImage, HObject* pHDisplayImage, HObject* pHDefectRgn_ImageNo, CString sBarcodeID, int* piNoSaveReviewImage)
{
	(void)iVisionCamType;
	...
	*piNoSaveReviewImage = 0;
	...
	try
	{
		...
		{
			strLog.Format("CInspectAdminViewHideDlg3::WriteSelectedRosReviewFile ==> Invalid Image");
			THEAPP.m_log_inspection->warn("{}", LOG_CSTR(strLog));
			return 0;
		}
		...
		for (size_t iGroup = 0; iGroup < vecReviewGroup.size(); iGroup++)
		{
			...
			else
			{
				if (THEAPP.Struct_PreferenceStruct.m_bReviewImageSaveLocal)
				{
					...
					THEAPP.m_pInspectService->AddListSaveReviewImageParam(pSaveThreadIDParam, VISION_NUMBER_3, m_pMzNo, m_pTrayNo, m_pModuleNo);
				}
				...
				if (THEAPP.Struct_PreferenceStruct.m_bReviewImageSaveLAS)
				{
					...
					THEAPP.m_pInspectService->AddListSaveReviewImageParam(pSaveThreadIDParam_Dual, VISION_NUMBER_3, m_pMzNo, m_pTrayNo, m_pModuleNo);
				}
			}
	...
	catch (HException& except)
	{
		...
		strLog.Format("Halcon Exception [InspectAdminViewHideDlg3::WriteSelectedRosReviewFile] : <%s>%s", sOperatorName, sErrMsg);
		THEAPP.m_log_halcon->warn("{}", LOG_CSTR(strLog));
...
int CInspectAdminViewHideDlg3::WriteSelectedRosReviewFile_FAI(int iMzNo, int iTrayNo, int iModuleNo, int iPcVisionNo, int iVisionCamType, int iNoInspectionImage, HObject* pHDisplayImage, HObject(*ppHReviewRgn_FAI_Item)[10], POINT(*ppiViewportCenter_FAI_Item)[10], CString sBarcodeID, int* piNoSaveReviewImage)
{
	CString sVisionCamType_Short;
	sVisionCamType_Short = THEAPP.m_ModelDefineInfo.m_strVisionName_Short[iPcVisionNo];
	...
	*piNoSaveReviewImage = 0;
	...
	try
	{
		if (THEAPP.m_pGFunction->ValidHImage(pHDisplayImage[0]) == FALSE)
		{
			strLog.Format("CInspectAdminViewHideDlg3::WriteSelectedRosReviewFile_FAI ==> Invalid Image");
			THEAPP.m_log_inspection->warn("{}", LOG_CSTR(strLog));
			return 0;
		}
	...
	catch (HException& except)
	{
		...
		strLog.Format("Halcon Exception [InspectAdminViewHideDlg3::WriteSelectedRosReviewFile_FAI] : <%s>%s", sOperatorName, sErrMsg);
		THEAPP.m_log_halcon->warn("{}", LOG_CSTR(strLog));
...
BOOL CInspectAdminViewHideDlg3::ImageCheck(CString sImagePath)
{
	try
	{
		HObject HCheckImage;
