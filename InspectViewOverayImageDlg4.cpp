...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewOverayImageDlg4 dialog

CInspectViewOverayImageDlg4* CInspectViewOverayImageDlg4::m_pInstance = NULL;

CInspectViewOverayImageDlg4* CInspectViewOverayImageDlg4::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance) {
		m_pInstance = new CInspectViewOverayImageDlg4();
		if (!m_pInstance->m_hWnd) {
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_INSPECT_VIEW_OVERAY_IMAGE4_DLG, pFrame->GetActiveView());
...
void CInspectViewOverayImageDlg4::DeleteInstance()
{
...
void CInspectViewOverayImageDlg4::Show()
{
	ShowWindow(SW_HIDE);
...
CInspectViewOverayImageDlg4::CInspectViewOverayImageDlg4(CWnd* pParent /*=NULL*/)
	: CDialog(CInspectViewOverayImageDlg4::IDD, pParent)
{
	//{{AFX_DATA_INIT(CInspectViewOverayImageDlg4)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
	SetPosition(VIEW1_DLG1_LEFT + VIEW1_DLG1_WIDTH + 285 + (VIEW1_DLG1_WIDTH + 280) / 2 - 8, VIEW1_DLG1_TOP + (VIEW1_DLG1_HEIGHT + 370) / 2 + 6 + VIEW_TOOLBAR_HEIGHT, (VIEW1_DLG1_WIDTH + 280) / 2 - 18, (VIEW1_DLG1_HEIGHT + 370) / 2 - 3 - VIEW_TOOLBAR_HEIGHT);
	...
	m_sWindowName = _T("Overlay Image 3rd");
	m_iCurResultNo = 3;
	ClearAll();
}

void CInspectViewOverayImageDlg4::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CInspectViewOverayImageDlg4)
	DDX_Control(pDX, IDC_VSCRBAR, m_VScrBar);
	DDX_Control(pDX, IDC_HSCRBAR, m_HScrBar);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CInspectViewOverayImageDlg4, CDialog)
	//{{AFX_MSG_MAP(CInspectViewOverayImageDlg4)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
...
/////////////////////////////////////////////////////////////////////////////
// CInspectViewOverayImageDlg4 message handlers

void CInspectViewOverayImageDlg4::ClearAll()
{
	m_dZoomRatio = 0.4;
...
BOOL CInspectViewOverayImageDlg4::PreTranslateMessage(MSG* pMsg)
{
	switch (pMsg->message) {
...
BOOL CInspectViewOverayImageDlg4::OnInitDialog()
{
...
BOOL CInspectViewOverayImageDlg4::OnEraseBkgnd(CDC *pDC)
{
	pDC->FillSolidRect(m_ClientRect, TS_COLOR_EXTRA_BG);
	return TRUE;
}

void CInspectViewOverayImageDlg4::OnPaint()
{
...
void CInspectViewOverayImageDlg4::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
void CInspectViewOverayImageDlg4::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (m_lWindowID < 0 || !m_bDisplayImage) return;
...
void CInspectViewOverayImageDlg4::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void CInspectViewOverayImageDlg4::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (!m_bDisplayImage) return;
...
void CInspectViewOverayImageDlg4::OnMouseMove(UINT nFlags, CPoint point)
{
	if (!m_bDisplayImage) return;
...
void CInspectViewOverayImageDlg4::OnDestroy()
{
	CDialog::OnDestroy();
...
void CInspectViewOverayImageDlg4::OnSize(UINT nType, int cx, int cy)
{
...
BOOL CInspectViewOverayImageDlg4::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	ScreenToClient(&pt);
...
BOOL CInspectViewOverayImageDlg4::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	switch (m_ToolBarState) {
	case TS_MOVE:
		if (m_bOnMoving)
...
void CInspectViewOverayImageDlg4::Reset()
{
	InitViewRect();
	UpdateViewportManager();
}

void CInspectViewOverayImageDlg4::InitViewRect()
{
	m_ViewRect.left = 0;
	m_ViewRect.top = 0;
...
void CInspectViewOverayImageDlg4::UpdateViewportManager()
{
...
void CInspectViewOverayImageDlg4::ViewRectSet()
{
...
void CInspectViewOverayImageDlg4::ScrollBarSet()
{
	SCROLLINFO Info;
...
void CInspectViewOverayImageDlg4::ScrollBarPosSet()
{
...
void CInspectViewOverayImageDlg4::DrawActiveTRegion()
{
...
void CInspectViewOverayImageDlg4::DrawSelectPartTRegion()
{
	CDC *pDC = GetDC();
...
void CInspectViewOverayImageDlg4::ZoomInSelectedPart(double dZoomRatio, int iStartPosX, int iStartPosY)
{
	if (m_bDisplayImage) {
...
void CInspectViewOverayImageDlg4::TbDraw()
{
	m_ToolBarState = TS_DRAW;
}

void CInspectViewOverayImageDlg4::TbMove()
{
	m_ToolBarState = TS_MOVE;
}

void CInspectViewOverayImageDlg4::TbSelectPart()
{
	m_ToolBarState = TS_SELECT_PART;
}

void CInspectViewOverayImageDlg4::TbZoomIn()
{
	if (m_bDisplayImage) {
...
void CInspectViewOverayImageDlg4::TbZoomOut()
{
	if (m_bDisplayImage) {
		m_dZoomRatio = (int)(m_dZoomRatio * 10) * 0.1;
...
void CInspectViewOverayImageDlg4::TbFitWidth()
{
	if (m_bDisplayImage)
	{
...
void CInspectViewOverayImageDlg4::TbFitHeight()
{
	if (m_bDisplayImage)
	{
		ClearWindow(m_lWindowID);
...
BOOL CInspectViewOverayImageDlg4::ShowWindow(int nCmdShow)
{
	return CDialog::ShowWindow(nCmdShow);
}

int CInspectViewOverayImageDlg4::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CInspectViewOverayImageDlg4::LoadScanImage(int nScanNum)
{

}

void CInspectViewOverayImageDlg4::MoveToDefectArea()
{

}

void CInspectViewOverayImageDlg4::RedrawView()
{
	InvalidateRect(NULL, FALSE);
}

void CInspectViewOverayImageDlg4::RedrawRect()
{
	InvalidateRect(&m_ClientRect, FALSE);
}

void CInspectViewOverayImageDlg4::UpdateView(BOOL bDelete)
{
	InvalidateRect(&m_ClientRect, bDelete);
}

void CInspectViewOverayImageDlg4::CallFitImageHeight()
{
	TbFitHeight();
}

void CInspectViewOverayImageDlg4::CallFitImageWidth()
{
	TbFitWidth();
}

void CInspectViewOverayImageDlg4::OnContextMenu(CWnd* pWnd, CPoint point)
{
	CMenu *pSubMenu;
...
void CInspectViewOverayImageDlg4::UpdateResultImage()
{
