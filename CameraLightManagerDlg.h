...
class CCameraLightManagerDlg : public CDialog
{
	DECLARE_DYNAMIC(CCameraLightManagerDlg)

protected:
	virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV 지원입니다.
	DECLARE_MESSAGE_MAP()

	static CCameraLightManagerDlg* m_pInstance;

	CButtonCS m_bnChangeLight;
	CButtonCS m_bnGrabSequence;
	CButtonCS m_bnChangeSpecular;
	CButtonCS m_bnLoadCalData;
...
public:
	CCameraLightManagerDlg(CWnd* pParent = nullptr);   // 표준 생성자입니다.
	virtual ~CCameraLightManagerDlg();

	static	CCameraLightManagerDlg* GetInstance(BOOL bShowFlag = FALSE);
	void	DeleteInstance();
	...
	// 대화 상자 데이터입니다.
	enum { IDD = IDD_CAMERA_LIGHT_MANAGER_DLG };
	...
	afx_msg void OnBnClickedButtonLoadCalData();
