// TrayAdminViewDlg4.cpp : 구현 파일입니다.
//
...
// CTrayAdminViewDlg4 대화 상자입니다.

CTrayAdminViewDlg4* CTrayAdminViewDlg4::m_pInstance = NULL;

CTrayAdminViewDlg4* CTrayAdminViewDlg4::GetInstance(BOOL bShowFlag)
{
	if (!m_pInstance)
	{
		m_pInstance = new CTrayAdminViewDlg4();
		if (!m_pInstance->m_hWnd)
		{
			CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
			m_pInstance->Create(IDD_TRAYVIEW_ADMIN_DLG4, pFrame->GetActiveView());
			if (bShowFlag)
				m_pInstance->Show();
		}
	}
...
void CTrayAdminViewDlg4::DeleteInstance()
{
...
IMPLEMENT_DYNAMIC(CTrayAdminViewDlg4, CDialog)

CTrayAdminViewDlg4::CTrayAdminViewDlg4(CWnd* pParent /*=NULL*/)
	: CDialog(CTrayAdminViewDlg4::IDD, pParent)
{
...
CTrayAdminViewDlg4::~CTrayAdminViewDlg4()
{

}

void CTrayAdminViewDlg4::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CTrayAdminViewDlg4, CDialog)
	ON_WM_WINDOWPOSCHANGING()
	ON_WM_PAINT()
END_MESSAGE_MAP()


// CTrayAdminViewDlg4 메시지 처리기입니다.


BOOL CTrayAdminViewDlg4::OnInitDialog()
{
	CDialog::OnInitDialog();

	m_TrayImagePB.m_iVisionIdx = VISION_NUMBER_4;
	m_TrayImagePB.m_iVisionCamType = VISION_NUMBER_4;

	if (THEAPP.Struct_PreferenceStruct.m_bUseVisionCopy)
		SetWindowText("Vision Copy #4");
	else
	{
...
void CTrayAdminViewDlg4::OnWindowPosChanging(WINDOWPOS* lpwndpos)
{
	//	MoveWindow(m_ScreenRect.left, m_ScreenRect.top, m_ScreenRect.right, m_ScreenRect.bottom);
...
void CTrayAdminViewDlg4::UpdateTrayDisplay()
{
	InvalidateRect(false);
}

void CTrayAdminViewDlg4::OnPaint()
{
...
BOOL CTrayAdminViewDlg4::PreTranslateMessage(MSG* pMsg)
{
...
void CTrayAdminViewDlg4::MakeTrayRectangle()
{
	try
	{
	...
	{
		strLog.Format("Halcon Exception [CTrayAdminViewDlg4::MakeTrayRectangle] : %s", (CString)(LPCTSTR)except.ErrorMessage());
