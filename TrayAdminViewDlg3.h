...
class CTrayAdminViewDlg3 : public CDialog, public ITrayAdminViewDlg
{
	DECLARE_DYNAMIC(CTrayAdminViewDlg3)

public:
	CTrayAdminViewDlg3(CWnd* pParent = NULL);   // 표준 생성자입니다.
	virtual ~CTrayAdminViewDlg3();

	// 대화 상자 데이터입니다.
	enum { IDD = IDD_TRAYVIEW_ADMIN_DLG3 };
	static CTrayAdminViewDlg3* m_pInstance;

public:
	static	CTrayAdminViewDlg3* GetInstance(BOOL bShowFlag = FALSE);
	void	DeleteInstance();
	void Show();
