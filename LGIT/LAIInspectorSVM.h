...
namespace LGIT {
	...
	public:
		...
		std::future<LGIT::Model::LAIInspectorSVMResult> add_job(
	...
	private:
		LGIT::Model::LAIInspectorSVMResult process_image(
			const std::string& product_name,
			const std::string& lot_id,
			const std::string& barcode,
			const int vision_pos,
	...
	private:
		...
		std::vector<std::shared_ptr<ModelInferenceSVM>> _models; // 모델 목록
