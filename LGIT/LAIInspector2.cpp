...
#include "Model/UpdateModelInfo.h" 
#include <exception>
...
namespace LGIT {
	...
	{
		std::string logger_name = "inspector2_module_logger";
		...
		std::string log_file = "Inspector2ModuleLog_" + get_current_date_string() + ".txt"; // 파일 싱크 설정
		...
		_logger->info("LAIInspector: num_threads={}, model_dir={}, inference_engine_logging_level={}",
			num_threads,
			model_dir,
		...
		try {
			for (size_t i = 0; i < num_threads; ++i) {
				_logger->info("ModelInference2: num_threads={}, model_dir={}", i, model_dir);
				auto model = std::make_shared<ModelInference2>(condition_name_list, condition_short_name_list, model_dir, div_type_idx);
				_models.push_back(model);
			}
			_inspector_initialize = true;
			_logger->info("LAIInspector2 initialized with {} models.", _models.size());
		}
		catch (const std::exception& e) {
			std::string error_msg = "Exception in LAIInspector2 constructor: ";
			_logger->error(error_msg + e.what());
			_inspector_initialize = false;
			_logger->error("Inspector2 models initialization failed: {}", error_msg);
	...
	std::future<LGIT::Model::LAIInspector2Result> LAIInspector2::add_job(
		...
		const cv::Mat& mask)
	{
		if (!_inspector_initialize) {
			_logger->error("Inspector2 is not initialized. Cannot add job.");
	...
	bool LAIInspector2::try_model_reload() {
		if (_inspector_initialize == false) {
			_logger->error("CLA models are not initialized.");
			return false;
		}
		if (_models.size() == 0) {
	...
	LGIT::Model::LAIInspector2Result LAIInspector2::process_image(
		const std::string& product_name,
		const std::string& lot_id,
		const std::string& barcode,
		const int vision_pos,
		...
		size_t idx)
	{
		...
		try {
			...
			LGIT::Model::LAIInspector2Result outputs = _models[idx]->RunInference(
				product_name, lot_id, barcode, 
			...
			std::string key = LGIT::ModelInference2::MakeInferenceEngineRunMemoryKey(product_name, _condition_name_list, vision_pos, image_no, roi_no, roi_inspection_type_idx, defect_name);
	...
	std::string LAIInspector2::get_current_date_string()
	{
