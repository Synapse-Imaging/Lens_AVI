...
UINT InspectionThread_SingleLens(LPVOID lp)
{
	...
	try
	{
		...
		while (TRUE)
		{
			...
			{
			case WAIT_OBJECT_0:
				strLog.Format("%s/ 정상: Interrupt 트리거 수신. 이벤트 신호 OK, LotID: %s, Port: %d, Tray: %d, Module: %d, Image: %d",
					sVisionCamType_Comm, sLotID, iMzNo, iTrayNo, iModuleNo, iNoCurImageGrab + 1);
				THEAPP.m_log_scan->info("{}", LOG_CSTR(strLog));
				break;
